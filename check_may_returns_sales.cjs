...
async function checkCrossMonthReturns() {
  ...
  try {
    ...
    console.log("Returns recorded in May for Sales before May:");
    console.table(rows);
    
    let sum = 0;
    for (const r of rows) sum += Number(r.total_credit_amount);
    console.log("Total extra credit polluting May:", sum.toFixed(3));
