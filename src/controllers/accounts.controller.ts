...
export const getFinancialSummary = async (req: Request, res: Response) => {
  try {
    const [income]: any = await pool.execute('SELECT SUM(total_amount) as total FROM sales_orders');
    const [expense]: any = await pool.execute('SELECT SUM(final_amount) as total FROM purchase_orders');
    const [returns]: any = await pool.execute('SELECT SUM(total_credit_amount) as total FROM sales_returns');
