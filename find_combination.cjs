...
async function run() {
  ...
  try {
    ...
    // Target counts from Excel
    const targets = {
      'Turkey Mozzarella': 340,
      'Halloumi Deli Sub': 177,
      'Grill Chicken & Mushroom': 111,
      'Egg & Cheese': 116,
      'Chicken Stroganoff': 81
    };

    let branchStats = {}; // branch -> item -> qty
    ...
    rows.forEach(r => {
      ...
      // Let's check both up to 30th and up to 31st
      // Actually let's just sum all of May to see the branch totals
      branchStats[branch][matchedItem] += Number(r.qty);
    });

    console.log("Branch Totals (May 1 - 31):");
    console.table(branchStats);
    
    // Now let's try to find exactly what combination yields the targets
    // It's a small set of branches, we can just print them all
