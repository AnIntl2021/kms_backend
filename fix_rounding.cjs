  ...
  try {
    ...
    for (const order of orders) {
      ...
      for (const item of items) {
        // Proper rounding: Use toFixed(3) to avoid JS floating point Math.round() bug
        const rawDiscounted = Number(item.price) * discountFactor;
        // Add Number.EPSILON to ensure exact half rounds up
        const discountedPrice = Math.round((rawDiscounted + Number.EPSILON) * 1000) / 1000;
    ...
    console.log("Successfully fixed the Javascript Math rounding bug!");
