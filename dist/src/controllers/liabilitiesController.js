...
export const createLiability = async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO company_liabilities (name, type, amount, interest_rate, due_date) VALUES (?, ?, ?, ?, ?)', [name, type || 'Loan', amount, interest_rate || '0%', due_date || null]);
...
export const updateLiability = async (req, res) => {
    ...
    const { name, type, amount, interest_rate, due_date } = req.body;
    try {
        await db.query('UPDATE company_liabilities SET name=?, type=?, amount=?, interest_rate=?, due_date=? WHERE liability_id=?', [name, type, amount, interest_rate, due_date, id]);
