    ...
    try {
        ...
        if (status === 'completed') {
            ...
            for (const item of items) {
                ...
                const [sourceStock] = await connection.execute('SELECT quantity FROM branch_stock WHERE branch_id = ? AND inventory_item_id = ?', [sourceBranchId, item.inventory_item_id]);
                ...
                await connection.execute('UPDATE branch_stock SET quantity = quantity - ? WHERE branch_id = ? AND inventory_item_id = ?', [item.quantity, sourceBranchId, item.inventory_item_id]);
                // Add to destination branch stock
                await connection.execute('INSERT INTO branch_stock (branch_id, inventory_item_id, quantity) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE quantity = quantity + ?', [transfer.to_branch_id, item.inventory_item_id, item.quantity, item.quantity]);
            }
        }
        // 3. Update status
