...
export const produceSandwiches = async (req, res) => {
    ...
    try {
        ...
        const [ingredients] = await connection.execute(`SELECT mii.inventory_item_id, mii.quantity, mii.package_id, ip.multiplier 
       ...
       WHERE mii.menu_item_id = ?`, [menu_item_id]);
        for (const ing of ingredients) {
            ...
            const [stock] = await connection.execute('SELECT current_stock, name_en FROM inventory_items WHERE inventory_item_id = ? FOR UPDATE', [ing.inventory_item_id]);
            ...
            await connection.execute('UPDATE inventory_items SET current_stock = current_stock - ? WHERE inventory_item_id = ?', [required, ing.inventory_item_id]);
        }
...
export const createSalesOrder = async (req, res) => {
    ...
    try {
        ...
        if (vendor_id && !resolvedCustomerName) {
            ...
            if (vendor.length > 0)
                resolvedCustomerName = vendor[0].name_en;
        }
        ...
        const [orderRes] = await connection.execute(`INSERT INTO sales_orders (order_number, vendor_id, branch_id, customer_name, total_amount, discount_percentage, discount_amount, final_amount, payment_method, payment_status, admin_id, salesman_id, batch_number, expiry_date, dispatch_status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [order_number || `SO-${Date.now()}`, vendor_id || null, branch_id && String(branch_id).trim().toLowerCase() === 'main' ? null : (branch_id || null), resolvedCustomerName || 'Counter Customer', totalAmount, discountPercentage, discountAmount, finalAmount, payment_method || 'credit', 'pending', admin_id, salesman_id || null, batch_number || null, sanitizedExpiryDate, 'pending', sanitizedDispatchDate]);
        ...
        for (const item of items) {
            ...
            await connection.execute(`INSERT INTO sales_order_items (sale_id, menu_item_id, quantity, price, expiry_date, batch_number) VALUES (?, ?, ?, ?, ?, ?)`, [sale_id, item.menu_item_id, item.quantity, item.price, sanitizedExpiryDate, batch_number || null]);
...
export const processReturn = async (req, res) => {
    ...
    try {
        ...
        const [returnRes] = await connection.execute('INSERT INTO sales_returns (sale_id, vendor_id, branch_id, reason, total_credit_amount, admin_id, salesman_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [sale_id || null, vendor_id, branch_id && String(branch_id).trim().toLowerCase() === 'main' ? null : (branch_id || null), reason || 'Expired', total_credit, admin_id, salesman_id || null, sanitizedReturnDate]);
      ...
      `, [current_sale_id, item.menu_item_id || item.product_id]);
            ...
            await connection.execute('INSERT INTO sales_return_items (return_id, menu_item_id, quantity, unit_price, expiry_date) VALUES (?, ?, ?, ?, ?)', [return_id, item.menu_item_id || item.product_id, item.quantity, item.price || item.unit_price, item.expiry_date ? String(item.expiry_date).split('T')[0] : null]);
...
export const updateSalesOrder = async (req, res) => {
    ...
    try {
        ...
        await connection.execute(`UPDATE sales_orders SET 
        vendor_id = ?, 
        branch_id = ?, 
        customer_name = ?, 
        total_amount = ?, 
      ...
      WHERE sale_id = ?`, [
        ...
        for (const item of items) {
            ...
            await connection.execute(`INSERT INTO sales_order_items (sale_id, menu_item_id, quantity, price, expiry_date, batch_number) VALUES (?, ?, ?, ?, ?, ?)`, [sale_id, item.menu_item_id, item.quantity, item.price, sanitizedExp, batch_number || null]);
            await connection.execute('UPDATE menu_items SET current_stock = current_stock - ? WHERE menu_item_id = ?', [item.quantity, item.menu_item_id]);
        }
        await connection.commit();
...
export const updateReturn = async (req, res) => {
    ...
    try {
        ...
        await connection.execute(`UPDATE sales_returns SET reason = ?, salesman_id = ?, total_credit_amount = ?${sanitizedReturnDate ? ', created_at = ?' : ''} WHERE return_id = ?`, [reason || 'Expired', salesman_id || null, total_credit, ...(sanitizedReturnDate ? [sanitizedReturnDate] : []), return_id]);
