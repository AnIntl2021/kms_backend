...
const getEmployees = async (req, res) => {
    try {
        const [rows] = await db_1.default.query('SELECT * FROM employees WHERE deleted_at IS NULL ORDER BY created_at DESC');
...
const createEmployee = async (req, res) => {
    ...
    try {
        ...
        const [result] = await db_1.default.query('INSERT INTO employees (employee_no, name, role, salary, allowances) VALUES (?, ?, ?, ?, ?)', [employee_no || null, name, role || 'Staff', salary || 0, allowancesJson]);
...
const updateEmployee = async (req, res) => {
    ...
    try {
        const allowancesJson = allowances ? JSON.stringify(allowances) : null;
        await db_1.default.query('UPDATE employees SET employee_no=?, name=?, role=?, salary=?, allowances=?, status=? WHERE employee_id=?', [employee_no || null, name, role, salary, allowancesJson, status || 'active', id]);
...
const deleteEmployee = async (req, res) => {
    const { id } = req.params;
    try {
        await db_1.default.query('UPDATE employees SET deleted_at = CURRENT_TIMESTAMP WHERE employee_id = ?', [id]);
