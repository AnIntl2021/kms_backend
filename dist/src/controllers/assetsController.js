...
export const createAsset = async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO company_assets (name, type, value, depreciation_rate, date_acquired) VALUES (?, ?, ?, ?, ?)', [name, type || 'General', value, depreciation_rate || 0, date_acquired || new Date()]);
...
export const updateAsset = async (req, res) => {
    ...
    const { name, type, value, depreciation_rate, date_acquired } = req.body;
    try {
        await db.query('UPDATE company_assets SET name=?, type=?, value=?, depreciation_rate=?, date_acquired=? WHERE asset_id=?', [name, type, value, depreciation_rate, date_acquired, id]);
