...
export const getSettings = async (req, res) => {
    try {
        ...
        if (tenantIdParam) {
            // Find tenant db
            const [tenants] = await pool.execute('SELECT db_name FROM tenants WHERE tenant_id = ? OR name = ?', [tenantIdParam, tenantIdParam]);
            if (tenants.length > 0) {
        ...
        await tenantContext.run({ dbName: targetDb }, async () => {
            ...
            if (targetDb && targetDb !== 'kms_master') {
                await tenantContext.run({ dbName: 'kms_master' }, async () => {
                    const [tenantRows] = await pool.execute('SELECT plan, name FROM tenants WHERE db_name = ?', [targetDb]);
                ...
                if (companyName) {
                    ...
                    await pool.execute('INSERT INTO system_settings (setting_key, setting_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE setting_value = ?', ['company_name', companyName, companyName]);
                    if (!settingsObj.receipt_header) {
                        settingsObj.receipt_header = companyName;
                        await pool.execute('INSERT INTO system_settings (setting_key, setting_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE setting_value = ?', ['receipt_header', companyName, companyName]);
                    }
                }
            }
            if (!settingsObj.receipt_footer) {
                settingsObj.receipt_footer = "Thank you for your visit!\nPlease come again.";
                await pool.execute('INSERT INTO system_settings (setting_key, setting_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE setting_value = ?', ['receipt_footer', settingsObj.receipt_footer, settingsObj.receipt_footer]);
            }
