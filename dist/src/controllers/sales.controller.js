...
exports.createPOSAddon = exports.getPOSAddons = exports.searchCustomers = exports.returnOrder = exports.deleteSale = exports.updateSaleStatus = exports.updatePaymentStatus = exports.createSale = exports.getSaleById = exports.getSales = void 0;
...
const createSale = async (req, res) => {
    ...
    try {
        const { branch_id, customer_name, client_phone, client_address, notes, items, total_amount, discount_amount = 0, discount_percentage = 0, table_number = null, order_type, payment_method, payment_status, counter_id, received_amount = null, returned_amount = null } = req.body;
        ...
        const [orderRes] = await connection.execute('INSERT INTO sales_orders (order_number, branch_id, order_type, payment_method, payment_status, customer_name, client_phone, client_address, notes, total_amount, discount_amount, discount_percentage, final_amount, table_number, status, admin_id, brand_id, counter_id, received_amount, returned_amount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', ['PENDING', branch_id || 1, order_type || 'walk_in', payment_method || 'cash', payment_status || (payment_method === 'credit' ? 'credit' : 'paid'), customer_name || null, client_phone || null, client_address || null, notes || null, total_amount, discount_amount, discount_percentage, final_amount, table_number, 'completed', admin_id, brandId, counter_id || null, received_amount, returned_amount]);
        ...
        for (const item of items) {
            ...
            await connection.execute('INSERT INTO sales_order_items (sale_id, menu_item_id, quantity, price, notes, addons) VALUES (?, ?, ?, ?, ?, ?)', [sale_id, item.menu_item_id, item.quantity, item.price, itemNotes, itemAddons]);
      ...
      `, [item.menu_item_id]);
            for (const ingredient of ingredients) {
                ...
                const [batches] = await connection.execute('SELECT batch_id, remaining_quantity FROM inventory_batches WHERE inventory_item_id = ? AND status = "active" ORDER BY created_at ASC FOR UPDATE', [ingredient.inventory_item_id]);
                for (const batch of batches) {
                    ...
                    await connection.execute('UPDATE inventory_batches SET remaining_quantity = ?, status = ? WHERE batch_id = ?', [newBatchQty, newBatchQty <= 0 ? 'exhausted' : 'active', batch.batch_id]);
                ...
                await connection.execute('UPDATE inventory_items SET current_stock = current_stock - ? WHERE inventory_item_id = ?', [totalDeduction, ingredient.inventory_item_id]);
                // Log movement
                await connection.execute('INSERT INTO audit_logs (admin_id, action, entity_name, entity_id, new_values) VALUES (?, ?, ?, ?, ?)', [admin_id, 'Stock Deduction (Sale)', 'InventoryItem', ingredient.inventory_item_id, JSON.stringify({ sale_id, deducted_qty: totalDeduction })]);
            }
        }
...
const deleteSale = async (req, res) => {
    ...
    try {
        ...
        const [items] = await connection.execute('SELECT menu_item_id, quantity FROM sales_order_items WHERE sale_id = ?', [id]);
      ...
      `, [item.menu_item_id]);
            for (const ingredient of ingredients) {
                ...
                await connection.execute('UPDATE inventory_items SET current_stock = current_stock + ? WHERE inventory_item_id = ?', [totalRestoration, ingredient.inventory_item_id]);
                // Restore to oldest active batch (FIFO logic)
                const [targetBatch] = await connection.execute('SELECT batch_id FROM inventory_batches WHERE inventory_item_id = ? AND status = "active" ORDER BY created_at ASC LIMIT 1', [ingredient.inventory_item_id]);
                ...
                // Log restoration
                await connection.execute('INSERT INTO audit_logs (admin_id, action, entity_name, entity_id, new_values) VALUES (?, ?, ?, ?, ?)', [admin_id, 'Stock Restoration (Delete Order)', 'InventoryItem', ingredient.inventory_item_id, JSON.stringify({ sale_id: id, restored_qty: totalRestoration })]);
            }
        }
        // 3. Mark as deleted
...
const returnOrder = async (req, res) => {
    ...
    try {
        ...
        await connection.beginTransaction();
        // 1. Get the items in this sale
        const [items] = await connection.execute('SELECT menu_item_id, quantity FROM sales_order_items WHERE sale_id = ?', [saleId]);
      ...
      `, [item.menu_item_id]);
            for (const ingredient of ingredients) {
                ...
                await connection.execute('UPDATE inventory_items SET current_stock = current_stock + ? WHERE inventory_item_id = ?', [totalRestoration, ingredient.inventory_item_id]);
                // Log restoration
                await connection.execute('INSERT INTO audit_logs (admin_id, action, entity_name, entity_id, new_values) VALUES (?, ?, ?, ?, ?)', [admin_id, 'Stock Restoration (Return)', 'InventoryItem', ingredient.inventory_item_id, JSON.stringify({ sale_id: saleId, restored_qty: totalRestoration })]);
            }
        }
...
const searchCustomers = async (req, res) => {
    try {
        const { q } = req.query;
        let query = `
      SELECT DISTINCT customer_name, client_phone 
      FROM sales_orders 
      WHERE customer_name IS NOT NULL 
    `;
        const params = [];
        if (q) {
            query += ` AND (customer_name LIKE ? OR client_phone LIKE ?)`;
            params.push(`%${q}%`, `%${q}%`);
        }
        query += ` LIMIT 50`;
        const [rows] = await db_js_1.default.execute(query, params);
        ...
        return (0, response_js_1.errorResponse)(res, 'Failed to search customers', 500, error);
    }
};
exports.searchCustomers = searchCustomers;
const getPOSAddons = async (req, res) => {
    try {
        const [rows] = await db_js_1.default.execute('SELECT * FROM pos_addons ORDER BY name ASC');
        return (0, response_js_1.successResponse)(res, rows);
    }
    catch (error) {
        console.error('Failed to fetch POS addons:', error);
        return (0, response_js_1.errorResponse)(res, 'Failed to fetch POS addons', 500, error);
    }
};
exports.getPOSAddons = getPOSAddons;
const createPOSAddon = async (req, res) => {
    try {
        const { name, price } = req.body;
        if (!name) {
            return (0, response_js_1.errorResponse)(res, 'Addon name is required', 400);
        }
        const cleanPrice = parseFloat(price) || 0;
        const [result] = await db_js_1.default.execute('INSERT INTO pos_addons (name, price) VALUES (?, ?) ON DUPLICATE KEY UPDATE price = VALUES(price)', [name.trim(), cleanPrice]);
        const addonId = result.insertId || null;
        return (0, response_js_1.successResponse)(res, { addon_id: addonId, name: name.trim(), price: cleanPrice }, 'Addon created/updated successfully');
    }
    catch (error) {
        console.error('Failed to create/update POS addon:', error);
        return (0, response_js_1.errorResponse)(res, 'Failed to create POS addon', 500, error);
    }
};
exports.createPOSAddon = createPOSAddon;
