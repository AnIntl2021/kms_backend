...
export const createBranch = async (req, res) => {
    try {
        ...
        const [countRows] = await pool.execute('SELECT COUNT(*) as count FROM branches WHERE deleted_at IS NULL');
        const currentCount = countRows[0].count;
        // Get database name to find tenant details
        const dbName = tenantContext.getStore()?.dbName || 'kms_master';
        if (dbName !== 'kms_master') {
            const [tenantRows] = await masterPool.execute('SELECT base_branches, extra_branches FROM tenants WHERE db_name = ?', [dbName]);
        ...
        const [result] = await pool.execute('INSERT INTO branches (name_en, name_ar, location_en, location_ar, phone, brand_id) VALUES (?, ?, ?, ?, ?, ?)', [name_en, name_ar, location_en || null, location_ar || null, phone || null, brandId]);
...
export const updateBranch = async (req, res) => {
    try {
        ...
        await pool.execute(`UPDATE branches SET name_en = ?, name_ar = ?, location_en = ?, location_ar = ?, phone = ?, status = ? 
