...
export const receivePurchaseOrder = async (req, res) => {
    ...
    try {
        ...
        if (orders[0].status !== 'pending' && orders[0].status !== 'partially_received')
            throw new Error('PO is not in a receivable state');
    ...
    `, [id]);
        ...
        for (const item of items) {
            ...
            await connection.execute('UPDATE inventory_items SET current_stock = current_stock + ? WHERE inventory_item_id = ?', [totalStockToAdd, item.inventory_item_id]);
            // 🛡️ INSERT NEW FIFO BATCH
            await connection.execute(`INSERT INTO inventory_batches (inventory_item_id, purchase_id, original_quantity, remaining_quantity, cost_per_unit) 
         VALUES (?, ?, ?, ?, ?)`, [item.inventory_item_id, id, totalStockToAdd, totalStockToAdd, costPerUnit]);
            // Log individual stock movements with multiplier context
            await connection.execute('INSERT INTO audit_logs (admin_id, action, entity_name, entity_id, new_values) VALUES (?, ?, ?, ?, ?)', [admin_id, 'FIFO Batch Purchase In', 'InventoryItem', item.inventory_item_id, JSON.stringify({
...
export const updatePurchaseOrder = async (req, res) => {
    ...
    try {
        ...
        await connection.execute(`UPDATE purchase_orders SET 
        vendor_id = ?, 
        branch_id = ?,
        invoice_type = ?,
        total_amount = ?, 
