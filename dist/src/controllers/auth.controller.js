...
export const login = async (req, res) => {
    try {
        ...
        if (tenantId) {
            ...
            const [tenantRows] = await pool.execute('SELECT db_name FROM tenants WHERE (tenant_id = ? OR name = ? OR db_name = ? OR contact_email = ?) AND status = "Active"', [tenantId, tenantId, tenantId, tenantId]);
            ...
            const [allTenants] = await pool.execute('SELECT db_name FROM tenants WHERE status = "Active"');
            let foundDb = null;
            // Loop through tenant databases to find where this email exists as an admin
            for (const tenant of allTenants) {
                try {
                    const [checkUser] = await pool.execute(`SELECT admin_id FROM \`${tenant.db_name}\`.admins WHERE (username = ? OR email = ?) AND deleted_at IS NULL AND status = 'active'`, [username, username]);
            ...
            if (foundDb) {
                ...
                const [checkMaster] = await pool.execute(`SELECT admin_id FROM admins WHERE (username = ? OR email = ?) AND deleted_at IS NULL AND status = 'active'`, [username, username]);
        ...
        tenantContext.run({ dbName: targetDb }, async () => {
            try {
                const [rows] = await pool.execute(`SELECT a.*, r.role_name, r.display_name_en, r.display_name_ar, r.permissions 
           FROM admins a 
           ...
           WHERE (a.username = ? OR a.email = ?) AND a.deleted_at IS NULL AND a.status = 'active'`, [username, username] // Check both fields with the same input
                ...
                await pool.execute('INSERT INTO audit_logs (admin_id, action, entity_name, ip_address) VALUES (?, ?, ?, ?)', [admin.admin_id, 'LOGIN', 'auth', req.ip]);
...
export const updateUser = async (req, res) => {
    try {
        ...
        if (password) {
            ...
            await pool.execute('UPDATE admins SET first_name=?, last_name=?, email=?, username=?, password=?, role_id=?, branch_id=?, status=? WHERE admin_id=?', [first_name, last_name, email, username, hashedPassword, role_id || null, branch_id || null, status || 'active', id]);
        }
        else {
            await pool.execute('UPDATE admins SET first_name=?, last_name=?, email=?, username=?, role_id=?, branch_id=?, status=? WHERE admin_id=?', [first_name, last_name, email, username, role_id || null, branch_id || null, status || 'active', id]);
        }
...
export const createUser = async (req, res) => {
    try {
        ...
        const [countRows] = await pool.execute('SELECT COUNT(*) as count FROM admins WHERE deleted_at IS NULL');
        const currentCount = countRows[0].count;
        const dbName = req.user?.tenant_db;
        if (dbName && dbName !== 'kms_master') {
            const [tenantRows] = await masterPool.execute('SELECT base_users, extra_users FROM tenants WHERE db_name = ?', [dbName]);
            if (tenantRows && tenantRows.length > 0) {
        ...
        // Check if exists
        const [existing] = await pool.execute('SELECT admin_id FROM admins WHERE username = ? OR email = ?', [username, email]);
        if (existing.length > 0) {
            return errorResponse(res, 'Username or email already exists.', 400);
        }
        const hashedPassword = await bcrypt.hash(password, 10);
        const [result] = await pool.execute('INSERT INTO admins (username, email, password, role_id, branch_id, first_name, last_name, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [username, email, hashedPassword, role_id, branch_id || null, first_name, last_name, status || 'active']);
        await pool.execute('INSERT INTO audit_logs (admin_id, action, entity_name, entity_id) VALUES (?, ?, ?, ?)', [req.user?.admin_id || 1, 'USER_CREATED', 'Admin', result.insertId]);
