...
export const updateUser = async (req, res) => {
    try {
        ...
        if (password) {
            ...
            await pool.execute('UPDATE admins SET first_name=?, last_name=?, email=?, username=?, password=?, role_id=?, status=? WHERE admin_id=?', [first_name, last_name, email, username, hashedPassword, role_id || null, status || 'active', id]);
        }
        else {
            await pool.execute('UPDATE admins SET first_name=?, last_name=?, email=?, username=?, role_id=?, status=? WHERE admin_id=?', [first_name, last_name, email, username, role_id || null, status || 'active', id]);
        }
...
export const createUser = async (req, res) => {
    try {
        ...
        const hashedPassword = await bcrypt.hash(password, 10);
        const [result] = await pool.execute('INSERT INTO admins (username, email, password, role_id, first_name, last_name, status) VALUES (?, ?, ?, ?, ?, ?, ?)', [username, email, hashedPassword, role_id, first_name, last_name, status || 'active']);
