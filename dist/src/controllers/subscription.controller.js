...
exports.closeSession = exports.getSessionSummary = exports.openSession = exports.getActiveSession = exports.deleteCounter = exports.updateCounter = exports.createCounter = exports.getCounters = exports.getBranchUsers = exports.verifySubscriptionPayment = exports.createSubscriptionOrder = exports.getSubscriptionStatus = void 0;
...
const getBranchUsers = async (req, res) => {
    ...
    try {
        const [rows] = await db_1.default.execute('SELECT admin_id, username, first_name, last_name FROM admins WHERE branch_id = ? AND deleted_at IS NULL AND status = "active"', [Number(branch_id)]);
        return (0, response_1.successResponse)(res, rows);
    }
    catch (error) {
        console.error('Get Branch Users Error:', error);
        return (0, response_1.errorResponse)(res, 'Failed to fetch branch users', 500, error);
    }
};
...
const createCounter = async (req, res) => {
    ...
    try {
        ...
        // 2. Insert counter
        const [result] = await db_1.default.execute('INSERT INTO pos_counters (branch_id, name, admin_id) VALUES (?, ?, ?)', [branch_id, name, admin_id || null]);
        return (0, response_1.successResponse)(res, { counter_id: result.insertId, branch_id, name, admin_id }, 'POS Counter created successfully');
    }
    catch (error) {
...
const updateCounter = async (req, res) => {
    ...
    try {
        await db_1.default.execute('UPDATE pos_counters SET branch_id = ?, name = ?, status = ?, admin_id = ? WHERE counter_id = ?', [branch_id, name, status || 'active', admin_id || null, id]);
        return (0, response_1.successResponse)(res, { counter_id: Number(id), branch_id, name, status, admin_id }, 'POS Counter updated successfully');
    }
    catch (error) {
        console.error('Update Counter Error:', error);
        return (0, response_1.errorResponse)(res, 'Failed to update POS counter', 500, error);
    }
};
...
const getSessionSummary = async (req, res) => {
    ...
    try {
       ...
       GROUP BY payment_method`, [session.branch_id, session.counter_id, openedAt, closedAt]);
        const breakdown = salesRows.map((r) => ({
            method: r.payment_method,
            amount: Number(r.amount)
        })).filter((r) => r.amount > 0);
        const cashRow = salesRows.find((r) => r.payment_method.toLowerCase() === 'cash');
        const cashSales = cashRow ? Number(cashRow.amount) : 0;
        const totalSales = salesRows.reduce((sum, r) => sum + Number(r.amount), 0);
